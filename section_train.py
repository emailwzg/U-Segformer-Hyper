...
from core.augmentations import ( PeruResize, 
...
def train(args):
    ...
    log_dir = os.path.join('runs-section', current_time +"_{}".format(args.arch))
    writer = SummaryWriter(log_dir=log_dir)
    # Setup Augmentations
    if args.aug:
        data_aug = Compose([PeruResize(),RandomRotate(10), RandomHorizontallyFlip(), AddNoise()])
    else:
        data_aug = None
    ...
    valdata_aug = Compose([PeruResize(),])
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            image_original, labels_original = images, labels # ([2, 1, 255, 701])， ([2, 1, 255, 701])
            images, labels = images.to(device), labels.to(device)

            optimizer.zero_grad()
            outputs = model(images) # 输入([2, 1, 255, 701])，输出([2, 6, 255, 701])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dim', nargs='?', type=int, default=64,
                        help='model_dim, use to segfomer and unet')
    parser.add_argument('--n_epoch', nargs='?', type=int, default=110,
                        help='# of the epochs')
    parser.add_argument('--batch_size', nargs='?', type=int, default=9,
                        help='Batch Size')
