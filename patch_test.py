...
def patch_label_2d(model, img, patch_size, stride, device):
    # img = (1, c, h, w)
    h, w = img.shape[-2:]
    ps = int(np.floor(patch_size/2))
    section_data = Section(img, patch_size, stride)
    bs = 256
    dl = DataLoader(section_data, batch_size=bs, num_workers=4, shuffle=False)

    num_classes = 6
    output_p = torch.zeros([1, num_classes, h+2*ps, w+2*ps])

    # generate output:
    for patches, positions in dl: # position为[xlist, ylist]
        patches = patches.to(device)
        model_output = model(patches) # (bs, 1, ps, ps) -> (bs, nc, ps, ps)
        model_output = F.interpolate(model_output, size=patches.shape[-2:], mode='bilinear', align_corners=False)
        model_output = model_output.detach().cpu() # (pred_bs, nc, ps, ps)
        positions = torch.stack(positions, dim=1) # (bs, 2)
        for output, (hdx, wdx) in zip(model_output, positions):
            # (nc, ps, ps), 
            output_p[0, :, hdx + ps: hdx + ps + patch_size, 
...
class Section(Dataset):
    def __init__(self, section, patch_size, stride):
        super(Section).__init__()
        self.patch_size = patch_size
        self.stride = stride
        ps = int(np.floor(patch_size/2))  # pad size 上下左右各填充49
        self.ps = ps

        img = section
        assert img.dim() == 4 and img.size(0)==1, "section must be (1, c, h, w)" 
        h, w = img.shape[-2:]  # height and width
        img_p = F.pad(img, pad=(ps, ps, ps, ps), mode='reflect')
        self.img_p = img_p

        hds = list(range(0, h-patch_size+ps, stride))
        wds = list(range(0, w-patch_size+ps, stride))
        self.positions = [(i, j) for i in hds for j in wds]

    def __len__(self):
        return len(self.positions)

    def __getitem__(self, i):
        hdx, wdx = self.positions[i]
        ps, patch_size = self.ps, self.patch_size
        patch = self.img_p[0, :, 
                        hdx + ps: hdx + ps + patch_size, # 加ps是原图相对pad后图的位置偏移
                        wdx + ps: wdx + ps + patch_size]
        return patch, (hdx, wdx) # (c, ps, ps)
...
def test(args):
    ...
    for sdx, split in enumerate(splits):
        ...
        numbers = [0, 99, 149, 200, 199, 499] ######对应/home/qnwang/code/F3seismic/data/splits/section_test1 和section_test2的剖面，如：200对应第201个剖面，test1中的x_0，test2中的i_200
        # testing mode:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', nargs='?', type=str, default='runs-patch/Aug28_155525_usegformer/usegformer_model.pkl',
    ...
    parser.add_argument('--parallel', nargs='?', type=bool, default=True,
                        help='whether to infer in parallel mode')
    ...
    parser.add_argument('--train_patch_size', nargs='?', type=int, default=128,
