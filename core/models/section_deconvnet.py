...
class section_deconvnet_skip(nn.Module):
    ...
    def __init__(self, n_classes=4, learned_billinear=False, **kwargs):
    ...
    def forward(self, x):
        size0 = x.size() # ([2, 1, 255, 701])
        conv1, indices1 = self.conv_block1(x) # [2, 64, 128, 351], [2, 64, 128, 351]
        size1 = conv1.size()
        conv2, indices2 = self.conv_block2(conv1) # [2, 128, 64, 176], [2, 128, 64, 176]
        size2 = conv2.size()
        conv3, indices3 = self.conv_block3(conv2) # [2, 256, 32, 88], [2, 256, 32, 88]
        size3 = conv3.size()
        conv4, indices4 = self.conv_block4(conv3) # [2, 512, 16, 44], [2, 512, 16, 44]
        size4 = conv4.size()
        conv5, indices5 = self.conv_block5(conv4) # [2, 512, 8, 22], [2, 512, 16, 44]  # 这个不一样

        conv6 = self.conv_block6(conv5) # [2, 4096, 6, 20]  # 减小了2，因为没有padding
        conv7 = self.conv_block7(conv6) # [2, 4096, 6, 20]
        conv8 = self.deconv_block8(conv7) + conv5  # [2, 512, 8, 22] # 增加了2，因为没有padding，其他deconvtrans都不变
        conv9 = self.unpool(conv8, indices5, output_size=size4) # [2, 512, 16, 44]
        conv10 = self.deconv_block10(conv9) + conv4   # [2, 512, 16, 44]
        conv11 = self.unpool(conv10, indices4, output_size=size3) # [2, 512, 32, 88]
        conv12 = self.deconv_block12(conv11) + conv3  # [2, 256, 32, 88]
        conv13 = self.unpool(conv12, indices3, output_size=size2)  # [2, 256, 64, 176]
        conv14 = self.deconv_block14(conv13) + conv2  # [2, 256, 64, 176]
        conv15 = self.unpool(conv14, indices2, output_size=size1)  # [2, 128, 128, 351]
        conv16 = self.deconv_block16(conv15) + conv1  # [2, 128, 128, 351]
        conv17 = self.unpool(conv16, indices1, output_size=size0)  # [2, 64, 255, 701]
        conv18 = self.deconv_block18(conv17)   # [2, 64, 255, 701]
        out = self.seg_score19(conv18)  # [2, 6, 255, 701]
